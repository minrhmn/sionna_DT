...
class cir_tx(gr.top_block, Qt.QWidget):

    def __init__(self, alpha=0.5, center_freq=3.5e9, degree=8, samp_rate=40e6):
        gr.top_block.__init__(self, "Spread Spectrum Tx", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Spread Spectrum Tx")
        ...
        self.settings = Qt.QSettings("GNU Radio", "cir_tx")
...
def argument_parser():
    ...
    parser.add_argument(
        "-f", "--center-freq", dest="center_freq", type=eng_float, default=eng_notation.num_to_str(float(3.5e9)),
        help="Set center_freq [default=%(default)r]")
    parser.add_argument(
        "-d", "--degree", dest="degree", type=intx, default=8,
...
def main(top_block_cls=cir_tx, options=None):
    ...
    tb = top_block_cls(alpha=options.alpha, center_freq=options.center_freq, degree=options.degree, samp_rate=options.samp_rate)
